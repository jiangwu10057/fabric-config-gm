...
type MSP struct {
	...
	IntermediateCerts []*x509GM.Certificate
	// Identity denoting the administrator of this MSP.
	Admins []*x509GM.Certificate
	...
	TLSRootCerts []*x509GM.Certificate
	// List of TLS intermediate certificates trusted by this MSP;
	// They are returned by GetTLSIntermediateCerts.
	TLSIntermediateCerts []*x509GM.Certificate
...
func (m *MSP) newMSPCRL(signingIdentity *SigningIdentity, certs ...*x509GM.Certificate) (*pkix.CertificateList, error) {
	...
	crl, err := x509GM.ParseCRL(crlBytes)
	if err != nil {
		return nil, err
	}
...
// signing certificate.
func (m *MSP) validateCertificates(signingCert *x509GM.Certificate, certs ...*x509GM.Certificate) error {
...
func (m *MSP) isCACert(signingCert *x509GM.Certificate) error {
...
func parseCertificateListFromBytes(certs [][]byte) ([]*x509GM.Certificate, error) {
	certificateList := []*x509GM.Certificate{}
...
func parseCertificateFromBytes(cert []byte) (*x509GM.Certificate, error) {
	pemBlock, _ := pem.Decode(cert)
	if pemBlock == nil {
		return &x509GM.Certificate{}, fmt.Errorf("no PEM data found in cert[% x]", cert)
	}

	certificate, err := x509GM.ParseCertificate(pemBlock.Bytes)
	if err != nil {
...
func parseCRL(crls [][]byte) ([]*pkix.CertificateList, error) {
	...
	for _, crl := range crls {
		...
		certificateList, err := x509GM.ParseCRL(pemBlock.Bytes)
		if err != nil {
...
func parsePrivateKeyFromBytes(priv []byte) (*sm2.PrivateKey, error) {
	if len(priv) == 0 {
		return nil, nil
	}
	...
	privateKey, err := x509GM.ParseSm2PrivateKey(pemBlock.Bytes)
	if err != nil {
...
func buildPemEncodedCertListFromX509(certList []*x509GM.Certificate) [][]byte {
	certs := [][]byte{}
...
func pemEncodeX509Certificate(cert *x509GM.Certificate) []byte {
	return pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: cert.Raw})
}

func pemEncodePKCS8PrivateKey(priv *sm2.PrivateKey) ([]byte, error) {
	privBytes, err := x509GM.MarshalSm2UnecryptedPrivateKey(priv)
	if err != nil {
...
func validateCACerts(caCerts []*x509GM.Certificate) error {
	for _, caCert := range caCerts {
		if (caCert.KeyUsage & x509GM.KeyUsageCertSign) == 0 {

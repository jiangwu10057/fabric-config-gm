...
func (o *OrdererOrg) CreateMSPCRL(signingIdentity *SigningIdentity, certs ...*x509GM.Certificate) (*pkix.CertificateList, error) {
	msp, err := o.MSP()
	if err != nil {
...
func unmarshalEtcdRaftMetadata(mdBytes []byte) (orderer.EtcdRaft, error) {
	...
	for _, c := range etcdRaftMetadata.Consenters {
		...
		clientTLSCert, err := x509GM.ParseCertificate(clientTLSCertBlock.Bytes)
		if err != nil {
		...
		serverTLSCert, err := x509GM.ParseCertificate(serverTLSCertBlock.Bytes)
		if err != nil {
